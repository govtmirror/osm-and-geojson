...
osm_geojson.geojson2osm = function(geo, changeset, osmChange) {
  ...
  case 'FeatureCollection':
    ...
    if (osmChange) {
      temp.osm = '<osmChange version="0.6" generator="github.com/aaronlidman/osm-and-geojson"><create>';
    }
...
osm_geojson.osm2geojson = function(osm, metaProperties) {
  ...
  function setProperties(element) {
    ...
    if (metaProperties) {
      setIf(element, 'user', props, 'updated_by');
      setIf(element, 'timestamp', props, 'updated_at');
    }
    ...
    for (var r = 0; r < relations_length; r++) {
      ...
      if (feature.properties.type == 'multipolygon') {
        ...
        for (var m = 0; m < members.length; m++) {
          if (attr(members[m], 'role') == 'outer') {
            assignWay(members[m], feature);
          }
        }

        for (var n = 0; n < members.length; n++) {
          if (attr(members[n], 'role') == 'inner') {
            assignWay(members[n], feature);
          }
        }
